<?php
ob_start();
$title = 'Gaji Dosen dibawah rata-rata';
include 'layout/header.php';

//deklarasi value umr
$query = "SELECT NIP, Nama, Gaji FROM dosen WHERE Gaji < (SELECT AVG(Gaji) FROM dosen) ORDER BY NIP ASC";
$filteredData = select($query);

// Periksa apakah filter sudah diterapkan
$filter = isset($_GET['filter']) ? $_GET['filter'] : 'below_avg';
...
<div class="container mt-5">
    ...
    <h1><i class="fas fa-database" style="color: #ff6600;"></i> Database Dosen<br>dengan Gaji dibawah rata-rata</h1>

    <?php if(empty($filteredData)) : ?>
        <p>Data tidak tersedia</p>
    <?php else : ?>
        <p>Rata-rata gaji= Rp<?= number_format($mean, 0, ',', '.'); ?></p>
        <p>Data Dosen dengan Gaji di bawah rata-rata: </p>
        <hr>
